...
//private ["_group","_countX","_countRanks","_LeaderX","_unitsX","_index","_positionX","_sideX","_typesX","_override","_canBypass"];
...
for "_i" from 0 to (_countX - 1) do
	...
	if ((_i == 0) or (((_allUnits + 1) < maxUnits) and ((_allUnitsSide + 1) < _maxUnitsSide)) or _override) then
		{
		_unit = _group createUnit [(_typesX select _i), _positionX, [], 0, "NONE"];
		...
		if ((_typesX select _i) in squadLeaders) then {_group selectLeader _unit};
		sleep 0.5;
		};
	};
