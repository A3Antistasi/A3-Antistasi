...
while {alive _dog} do
	{
	if ((_dog == leader _group) and (!captive _dog)) then {[_dog,true] remoteExec ["setCaptive",0,_dog]; _dog setCaptive true};
	if (isNull _spotted) then
		...
		if (_dog distance (leader _group) > 50) then {_dog setPos position (leader _group)};
		}
	else
		{
