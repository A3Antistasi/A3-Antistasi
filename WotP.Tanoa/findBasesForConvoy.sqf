...
_aeropuertosAAF = (aeropuertos - mrkSDK) select {(spawner getVariable _x == 2) and (dateToNumber date > server getVariable _x) and ([_x,_marcador] call isTheSameIsland) and (!(_x in forcedSpawn))};
if (_marcador in ciudades) then {_aeropuertosAAF = _aeropuertosAAF - mrkCSAT};
_aeropuertos = [];
_base = "";
{
_base = _x;
