...
if (_medico != _unit) then
	...
	if ((isPlayer _unit) and !(isMultiplayer))  then
		{
		if (([_medico] call canFight) and (_medico distance _unit > 3) and (_medico == _unit getVariable ["ayudado",objNull]) and !(_unit getVariable ["llevado",false])) then {_medico setPos position _unit};
		};
