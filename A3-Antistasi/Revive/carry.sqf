...
waitUntil {sleep 0.5; (!alive _llevado) or !([_llevador] call canFight) or !(_llevado getVariable ["INCAPACITATED",false]) or ({!isNull _x} count attachedObjects _llevador == 0) or (time > _timeOut) or (vehicle _llevador != _llevador)};
