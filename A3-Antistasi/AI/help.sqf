...
if (_medico != _unit) then
	...
	if ((isPlayer _unit) and !(isMultiplayer))  then
		{
		if (([_medico] call A3A_fnc_canFight) and (_medico distance _unit > 3) and (_medico == _unit getVariable ["helped",objNull]) and !(_unit getVariable ["carryX",false]) and (allUnits findIf {((side _x == malos) or (side _x == Invaders)) and (_x distance2D _unit < 50)} == -1)) then {_medico setPos position _unit};
		};
