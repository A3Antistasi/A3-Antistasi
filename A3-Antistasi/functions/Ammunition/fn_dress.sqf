...
_loadout = switch _team do {
	...
	case teamPlayer: {
		if (toLower worldName isEqualTo "enoch") then {
			[[],[],[],[selectRandom ((faction_civ getVariable "uniforms") + (faction_reb getVariable "uniforms")), []],[],[],"H_Hat_Tinfoil_F","",[],
			[(selectRandom unlockedmaps),"","",(selectRandom unlockedCompasses),(selectRandom unlockedwatches),""]];
		} else {
			[[],[],[],[selectRandom ((faction_civ getVariable "uniforms") + (faction_reb getVariable "uniforms")), []],[],[],selectRandom faction_civ getVariable "headgear","",[],
			[(selectRandom unlockedmaps),"","",(selectRandom unlockedCompasses),(selectRandom unlockedwatches),""]];
		};
	};
