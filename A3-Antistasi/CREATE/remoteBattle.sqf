...
waitUntil {sleep 10;{([_x] call canFight) and (vehicle _x == _x)} count _soldados == {[_x] call canFight} count _soldados};

if ({[_x] call canFight} count _soldados == 0) exitWith {};
...
while {true} do
	{
	sleep 10;//poner 10
	_soldados = _soldados select {[_x] call canFight};
	...
	if ((_x distance _soldado < (2*distanciaSPWN)) and (isPlayer _x)) then
		...
		{
		if ((_x distance _soldado < (distanciaSPWN/2)) and {[_x] call canFight} and {side group _x in _eny} and {vehicle _x == _x}) then
			{
