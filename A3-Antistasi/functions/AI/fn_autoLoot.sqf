...
if (_unit distance _objectX < _distanceX) then
	...
	if ((count weaponCargo _objectX > 0) and !(_objectX getVariable ["busy",false])) then
		...
		for "_i" from 0 to (count _weaponsX - 1) do
			...
			//if ((not(_basePossible in unlockedWeapons)) and ((_basePossible in allRifles) or (_basePossible in srifles) or (_basePossible in mguns) or (_potential in mlaunchers) or (_potential in allRocketLaunchers))) then
			if ((_basePossible in allRifles) or (_basePossible in srifles) or (_basePossible in mguns) or (_potential in mlaunchers) or (_potential in allRocketLaunchers)) then
				{
				_target = _objectX;
