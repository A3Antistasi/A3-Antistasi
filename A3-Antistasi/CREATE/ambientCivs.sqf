...
while { true } do {
    ...
    if ((count _allCivs < civPerc) and 
            ({(local _x) and (simulationEnabled _x) and (alive _x)} count allUnits < maxUnits)) then {
        if (_reset) then {
            _casas = (nearestTerrainObjects[player, ["House"], 300]) select {
            ...
            if ({ _x distance player < 300 } count _allCivs <= _numCasas) then {
                ...
                if ({ _x distance _casa < 20 } count(allPlayers - (entities "HeadlessClient_F")) == 0) then {
                    ...
                    if (_civ == leader _civ) then {
                        _nul = [_civ, "ambientCiv", "SAFE", "SPAWNED", "NOFOLLOW", "NOVEH2", "NOSHARE", "DoRelax"] execVM "scripts\UPSMON.sqf"
                    };
                };
            };
        };
