...
private ["_pos","_roadscon","_veh","_roads","_conquered","_dirVeh","_markerX","_positionX","_vehiclesX","_soldiers","_tam","_bunker","_grupoE","_unit","_typeGroup","_grupo","_timeLimit","_dateLimit","_dateLimitNum","_base","_perro","_lado","_cfg","_esFIA","_salir","_isControl","_tam","_tipoVeh","_tipoUnit","_markersX","_frontierX","_uav","_groupUAV","_allUnits","_closest","_winner","_timeLimit","_dateLimit","_dateLimitNum","_size","_base","_mina","_loser","_lado"];
	...
	if (_frontierX) then
		...
		if ({if (_x inArea _markerX) exitWith {1}} count allMines == 0) then
			...
			for "_i" from 1 to 60 do
				...
				if (_lado == malos) then {malos revealMine _mina} else { revealMine _mina};
				};
			};
	...
	if (_indice > defaultControlIndex) then
		...
		if ((!_isControl) and (_winner == buenos)) then
			...
			for "_i" from 1 to 60 do
				{
				_mina = createMine ["APERSMine",_positionX,[],_size];
				if (_loser == malos) then {malos revealMine _mina} else { revealMine _mina};
				};
			};
		*/
		};
