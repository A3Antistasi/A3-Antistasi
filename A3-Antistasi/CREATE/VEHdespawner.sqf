...
if ((typeOf _veh in arrayCivVeh) and ({(_x getVariable ["spawner",false]) and (side group _x == teamPlayer)} count crew _veh > 0) and (_veh distance getMarkerPos respawnTeamPlayer > 50)) then
	...
	if (random 100 < _prestigeOPFOR) then
		...
		{
		if ((side _x == Occupants) and (_x distance _pos < distanceSPWN)) then {_x reveal [_amigo,4]};
		} forEach allUnits;
		} forEach crew _veh;
		};
	};
while {alive _veh} do
	{
	if ((not([distanceSPWN,1,_veh,teamPlayer] call A3A_fnc_distanceUnits)) and (not([distanceSPWN,1,_veh,] call A3A_fnc_distanceUnits)) and (not([distanceSPWN,1,_veh,Occupants] call A3A_fnc_distanceUnits)) and (not(_veh in staticsToSave)) and (_veh distance getMarkerPos respawnTeamPlayer > 100)) then
		{
