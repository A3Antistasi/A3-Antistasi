...
if (vehicle _player != _player) then
	...
	if (vehicle _player in reportedVehs) then
		{
		hint "This vehicle has been reported to the enemy. Change or renew it in the Garage to become Undercover";
		_cambiar = "Init";
		};
	}
else
	...
	if ((primaryWeapon _player != "") or (secondaryWeapon _player != "") or (handgunWeapon _player != "") or (vest _player != "") or (getNumber (configfile >> "CfgWeapons" >> headgear _player >> "ItemInfo" >> "HitpointsProtectionInfo" >> "Head" >> "armor") > 2) or (hmd _player != "") or (not(uniform _player in civUniforms))) then
		{
		hint "You cannot become Undercover while:\n\n- A weapon is visible\n- Wearing Vest\n- Wearing Helmet\n- Wearing NVGs\n- Wearing Mil Uniform";
		_cambiar = "Init";
		};
	if (dateToNumber date < _compromised) then
		{
		hint "You have been reported in the last 30 minutes therefor you cannot become Undercover";
		_cambiar = "Init";
		};
	};
...
switch _cambiar do
	...
	case "VCompromised": {hint "You entered a reported vehicle"};
	case "SpotBombTruck":
		{
	...
	case "Compromised": {hint "You left your vehicle and are still on the Wanted List"};
	case "Distancia":
		{
