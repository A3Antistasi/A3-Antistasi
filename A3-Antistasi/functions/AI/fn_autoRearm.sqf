		...
		if (_unit distance _objectX < _distanceX) then
			...
			if ((count weaponCargo _objectX > 0) and !(_objectX getVariable ["busy",false])) then
				...
				for "_i" from 0 to (count _weaponsX - 1) do
					...
					if ((not(_basePossible in ["hgun_PDW2000_F","hgun_Pistol_01_F","hgun_ACPC2_F","arifle_AKM_F","arifle_AKS_F","SMG_05_F","LMG_03_F"])) and ((_basePossible in allRifles) or (_basePossible in srifles) or (_basePossible in allMachineGuns))) then
						{
						_target = _objectX;
						_hasBox = true;
