...
private ["_cosa","_jugador","_id","_sitios","_markerX","_size","_positionX"];
...
_markerX = [_sitios,_jugador] call BIS_fnc_nearestPosition;
_size = [_markerX] call A3A_fnc_sizeMarker;
_positionX = getMarkerPos _markerX;
if (_jugador distance2D _positionX > _size) exitWith {hint "This asset needs to be closer to it relative zone center to be able to be moved"};
...
waitUntil {sleep 1; (count attachedObjects _jugador == 0) or (vehicle _jugador != _jugador) or (_jugador distance2D _positionX > (_size-3)) or !([_jugador] call A3A_fnc_canFight) or (!isPlayer _jugador)};
...
if  (_jugador distance2D _positionX > _size) exitWith {hint "This asset cannot be moved more far away for its zone center"};