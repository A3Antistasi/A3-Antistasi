...
private ["_grupo","_veh","_texto","_unitsX"];
...
_texto = format ["%1 Status\n\nAlive members: %2\nAble to combat: %3\nCurrent task: %4\nCombat Mode:%5\n",groupID _grupo,{alive _x} count _unitsX,{[_x] call A3A_fnc_canFight} count _unitsX,_grupo getVariable ["tarea","Patrol"],behaviour (leader _grupo)];
if ({[_x] call A3A_fnc_isMedic} count _unitsX > 0) then {_texto = format ["%1Operative Medic\n",_texto]} else {_texto = format ["%1No operative Medic\n",_texto]};
if ({_x call A3A_fnc_typeOfSoldier == "ATMan"} count _unitsX > 0) then {_texto = format ["%1With AT capabilities\n",_texto]};
if ({_x call A3A_fnc_typeOfSoldier == "AAMan"} count _unitsX > 0) then {_texto = format ["%1With AA capabilities\n",_texto]};
if (!(isNull(_grupo getVariable ["mortarsX",objNull])) or ({_x call A3A_fnc_typeOfSoldier == "StaticMortar"} count _unitsX > 0)) then
	{
	if ({vehicle _x isKindOf "StaticWeapon"} count _unitsX > 0) then {_texto = format ["%1Mortar is deployed\n",_texto]} else {_texto = format ["%1Mortar not deployed\n",_texto]};
	}
else
	{
	if ({_x call A3A_fnc_typeOfSoldier == "StaticGunner"} count _unitsX > 0) then
		{
		if ({vehicle _x isKindOf "StaticWeapon"} count _unitsX > 0) then {_texto = format ["%1Static is deployed\n",_texto]} else {_texto = format ["%1Static not deployed\n",_texto]};
		};
	};
...
if !(isNull _veh) then
	...
	if (!alive _veh) then
		...
		_texto = format ["%1Boarded:%2/%3",_texto,{vehicle _x == _veh} count _unitsX,{alive _x} count _unitsX];
		};
	};
