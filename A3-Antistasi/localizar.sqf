...
if (_sitio in citiesX) then
	...
	_city = [_city,false] call A3A_fnc_fn_location;
	if (_sitio in airportsX) then {_texto = format ["%1 Airbase",_city]};
	if (_sitio in resourcesX) then {_texto = format ["Resource near %1",_city]};
	if (_sitio in factories) then {_texto = format ["Factory near %1",_city]};
	if (_sitio in outposts) then {_texto = format ["Outpost near %1",_city]};
	if (_sitio in seaports) then {_texto = format ["Seaport near %1",_city]};
