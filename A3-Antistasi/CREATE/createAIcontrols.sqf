...
private ["_pos","_roadscon","_veh","_roads","_conquered","_dirVeh","_markerX","_positionX","_vehiclesX","_soldiers","_tam","_bunker","_groupE","_unit","_typeGroup","_group","_timeLimit","_dateLimit","_dateLimitNum","_base","_dog","_sideX","_cfg","_isFIA","_leave","_isControl","_tam","_typeVehX","_typeUnit","_markersX","_frontierX","_uav","_groupUAV","_allUnits","_closest","_winner","_timeLimit","_dateLimit","_dateLimitNum","_size","_base","_mineX","_loser","_sideX"];
...
if (_isControl) then
	...
	if (!_isFIA) then
		...
		_group = if !(hasIFA) then {[_positionX,_sideX, _typeGroup,false,true] call A3A_fnc_spawnGroup} else {[_positionX,_sideX, _typeGroup] call A3A_fnc_spawnGroup};
	...
	if (_frontierX) then
		...
		if ({if (_x inArea _markerX) exitWith {1}} count allMines == 0) then
			...
			for "_i" from 1 to 60 do
				...
				if (_sideX == Occupants) then {Occupants revealMine _mineX} else { revealMine _mineX};
				};
			};
		_group = [_positionX,_sideX, _cfg] call A3A_fnc_spawnGroup;
	...
	if (_indexX > defaultControlIndex) then
		...
		if ((!_isControl) and (_winner == teamPlayer)) then
			...
			for "_i" from 1 to 60 do
				{
				_mineX = createMine ["APERSMine",_positionX,[],_size];
				if (_loser == Occupants) then {Occupants revealMine _mineX} else { revealMine _mineX};
				};
			};
		*/
		};
