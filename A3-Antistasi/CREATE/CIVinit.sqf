...
_EHkilledIdx = _unit addEventHandler ["killed",
	...
	if (_muerto == _killer) then
		{
		_nul = [-1,-1,getPos _muerto, "Civ Killed"] remoteExec ["A3A_fnc_citySupportChange",2];
		}
	else
		{
		...
		if (side _killer == buenos) then
			...
			_nul = [5,-5,getPos _muerto, "Civ Killed by SDK"] remoteExec ["A3A_fnc_citySupportChange",2];
			}
		else
			{
