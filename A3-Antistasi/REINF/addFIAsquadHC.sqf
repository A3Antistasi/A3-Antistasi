...
private ["_typeGroup","_esinf","_typeVehX","_costs","_costHR","_exit","_formatX","_pos","_hr","_resourcesFIA","_group","_roads","_road","_group","_truckX","_vehicle","_mortarX","_morty"];
...
private ["_group","_roads","_truckX","_withBackpck"];
_withBackpck = "";
if (_typeGroup isEqualType []) then
	{
	{
...
if (_esinf) then
	...
	if (_typeGroup isEqualType []) then
		{
		_group = [_pos, teamPlayer, _formatX,true] call A3A_fnc_spawnGroup;
		...
		if (_withBackpck == "MG") then
			{
			((units _group) select ((count (units _group)) - 2)) addBackpackGlobal supportStaticsSDKB2;
			...
			if (_withBackpck == "Mortar") then
				{
				((units _group) select ((count (units _group)) - 2)) addBackpackGlobal supportStaticsSDKB3;
	...
	if ((!activeGREF) and (_typeGroup == staticAAteamPlayer)) then
		...
		_morty = _group createUnit [staticCrewTeamPlayer, _pos, [],0, "NONE"];
	...
	if (_typeGroup == vehSDKAT) then {_group setGroupId [format ["M.AT-%1",{side (leader _x) == teamPlayer} count allGroups]]};
	if (_typeGroup == staticAAteamPlayer) then {_group setGroupId [format ["M.AA-%1",{side (leader _x) == teamPlayer} count allGroups]]};
...
_mortarX setVariable ["owner",_group,true];
_nul = [0, - _costs] remoteExec ["A3A_fnc_resourcesFIA",2];
leader _group assignAsDriver _mortarX;
