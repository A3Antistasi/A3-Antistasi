			...
			for "_i" from 0 to ((count _sites) - 1) do
				...
				if ((_pos distance _posbase < (distanceMission*2)) and (_base !="")) then
					...
					if ((_siteX in citiesX) and (sidesX getVariable [_siteX,sideUnknown] == teamPlayer)) then
						...
						{
						if (((sidesX getVariable [_siteX,sideUnknown] == Occupants) and (sidesX getVariable [_base,sideUnknown] == Occupants)) or ((sidesX getVariable [_siteX,sideUnknown] == ) and (sidesX getVariable [_base,sideUnknown] == ))) then {_potentials pushBack _siteX};
						};
					};
				};
			};
