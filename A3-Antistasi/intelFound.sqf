...
if (count _this == 1) then
	...
	if (_markerX isEqualType "") then
		...
		if (sidesX getVariable [_markerX,sideUnknown] == ) then {_sideX = };
		}
	else
		{
...
if (_sideX == Occupants) then {_minesAAF = _minesAAF - (detectedMines )} else {_minesAAF = _minesAAF - (detectedMines Occupants)};
_revealMineX = false;
