...
for "_i" from 0 to ((count _units) - 1) do
{
	...
	if (_vehicle != objNull) then {
		...
		if(_vehicle isKindOf "Air") then
		{
			...
			private _fsm = [_vehicle, _airOffset, _markers, _convoyType] execFSM QPATHTOFOLDER(FSMs\ConvoyTravelAir.fsm);
		...
		else
		{
			...
			private _fsm = [_vehicle, _route, _markers, _convoyType] execFSM QPATHTOFOLDER(FSMs\ConvoyTravel.fsm);
			_vehicle setVariable ["fsm", _fsm];
		};
