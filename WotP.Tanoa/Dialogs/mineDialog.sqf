...
_coste = (2*(server getVariable (SDKExp select 0))) + ([vehSDKTruck] call vehiclePrice);
_hr = 2;
...
if (_tipo == "delete") exitWith
	...
	[] remoteExec ["mineSweep",call AS_fnc_getNextWorker];
	};
...
[_tipo,_posicionTel,_cantidad] remoteExec ["buildMinefield",call AS_fnc_getNextWorker];