...
if (_tam < ([_marcador] call A3A_fnc_garrisonSize)) then {
    ...
    if ({if ((getMarkerPos _x inArea _mrk) and (lados getVariable[_x, sideUnknown] != _lado)) exitWith {1} } count marcadores > 0) then {
        _patrol = false
    };
};
if (_patrol) then {
    ...
    while { (spawner getVariable _marcador != 2) and (_cuenta < 4) } do {
        ...
        if ([_marcador, false] call A3A_fnc_fogCheck < 0.3) then {
            _arrayGrupos = _arrayGrupos - sniperGroups
        };
        ...
        if !(isNull _grupo) then {
            ...
            {
                [_x, _marcador] call A3A_fnc_NATOinit;
...
{
    _nul = [_x] call A3A_fnc_AIVEHinit;
} forEach _vehiculos;
_roads = _posicion nearRoads (_size * 2) max (200);
...
if ((_marcador in puertos) and (spawner getVariable _marcador != 2) and !hayIFA) then {
    ...
    if ([_tipoVeh] call A3A_fnc_vehAvailable) then {
        _mrkMar = seaSpawn select {
            getMarkerPos _x inArea _marcador
        };
    ...
    //if (_frontera) then {
        if (spawner getVariable _marcador != 2) then {
            if (count _roads != 0) then {
                _roads_remaining = _roads apply { [position _x distance _posicion, _x] };
                _roads_remaining sort false;
                private _itr = 0;
                while {count _roads_remaining > 0 and _itr < 4} do {
                    _dist = _roads_remaining select 0 select 0;
                    _road = _roads_remaining select 0 select 1; 
                    // objNull;
                    // {
                    //     if ((position _x) distance _posicion > _dist) then {
                    //         _road = _x;
                    //         _dist = position _x distance _posicion
                    //     }
                    // } forEach _roads_remaining;
                    _roadscon = (roadsConnectedto _road) apply { [position _x distance _posicion, _x] };
                    _roadscon sort false;
                    if (count _roadscon > 0) then {
                        _roadcon = _roadscon select 0 select 1; 
                        // {
                        //     if ((position _x) distance _posicion > _dist) then {
                        //         _roadcon = _x
                        //     }
                        // } forEach _roadscon;
                        _dirveh = _roadcon getDir _road;
                        if (!_esFIA) then {
                            private _offs = [7, -7];
                            {
                                _grupo = createGroup _lado;
                                _grupos pushBack _grupo;
                                _pos = [getPos _road, _x, _dirveh + 270] call BIS_Fnc_relPos;
                    ...
                    _roads_remaining = _roads_remaining select {
                        ((getPos _road) vectorDiff _posicion) vectorCos ((getPos (_x select 1)) vectorDiff _posicion) < 0.3 
                    };
                    _itr = _itr + 1;
                };
            };
...
for "_i" from 0 to(count _array - 1) do {
    _grupo =
        if (_i == 0) then {
            [_posicion, _lado, (_array select _i), true, false] call A3A_fnc_spawnGroup
        } else {
            [_posicion, _lado, (_array select _i), false, true] call A3A_fnc_spawnGroup
        };
...
waitUntil {
    sleep 1;
    (spawner getVariable _marcador == 2)
};
