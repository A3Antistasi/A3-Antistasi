...
while {visibleMap} do
	...
	if (count positionTel > 0) then
		...
		_nameFaction = if (lados getVariable [_sitio,sideUnknown] == teamPlayer) then {nameTeamPlayer} else {if (lados getVariable [_sitio,sideUnknown] == malos) then {nameOccupants} else {nameInvaders}};
