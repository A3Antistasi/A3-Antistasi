...
private ["_markerX","_positionX","_dateLimit","_dateLimitNum","_nameDest","_typeVehX","_textX","_truckCreated","_size","_pos","_veh","_group","_unit"];
...
if (spawner getVariable _markerX == 0) then
	...
	for "_i" from 1 to 3 do
		{
		_unit = _group createUnit [_typeX, _pos, [], 0, "NONE"];
	...
	if ((not alive _veh) or ({(_x getVariable ["spawner",false]) and (side group _x == teamPlayer)} count crew _veh > 0)) then
		...
		if (_sideX == ) then {[0,3] remoteExec ["A3A_fnc_prestige",2]; [0,10*_bonus,_positionX] remoteExec ["A3A_fnc_citySupportChange",2]} else {[3,0] remoteExec ["A3A_fnc_prestige",2];[0,5*_bonus,_positionX] remoteExec ["A3A_fnc_citySupportChange",2]};
